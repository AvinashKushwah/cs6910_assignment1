...
parser.add_argument('-wp' , '--wandb_project', help='Project name used to track experiments in Weights & Biases dashboard' , type=str, default='dl_assignment')
...
parser.add_argument('-e', '--epochs', help="Number of epochs to train neural network.", type=int, default=20)
parser.add_argument('-b', '--batch_size', help="Batch size used to train neural network.", type=int, default=64)
parser.add_argument('-l','--loss', help = 'choices: ["mse", "cross_entropy"]' , type=str, default='cross_entropy')
parser.add_argument('-o', '--optimizer', help = 'choices: ["sgd", "momentum", "nestrov", "rmsprop", "adam", "nadam"]', type=str, default = 'adam')
...
parser.add_argument('-beta', '--beta', help='Beta used by rmsprop optimizer',type=float, default=0.85)
parser.add_argument('-beta1', '--beta1', help='Beta1 used by adam and nadam optimizers.',type=float, default=0.8)
parser.add_argument('-beta2', '--beta2', help='Beta2 used by adam and nadam optimizers.',type=float, default=0.9)
parser.add_argument('-eps', '--epsilon', help='Epsilon used by optimizers.',type=float, default=0.0000001)
...
parser.add_argument('-sz', '--hidden_size', help ='Number of hidden neurons in a feedforward layer.', nargs='+', type=int, default=256, required=False)
...
neuralNetwork(epochs = arg.epochs,batch_size = arg.batch_size,optimizer=arg.optimizer,learning_rate=arg.learning_rate,beta=arg.beta,beta1=arg.beta1,beta2=arg.beta2,epsilon=arg.epsilon,weight_init=arg.weight_init,num_layers=arg.num_layers,hidden_size=arg.hidden_size,activation=arg.activation,momentum=arg.momentum,loss_function=arg.loss,weight_decay=arg.weight_decay)
#

#neuralNetwork(epochs,batch_size,optimizer,learning_rate,beta,beta1,beta2,epsilon,weight_init,num_layers,hidden_size,activation,momentum,loss_function,weight_decay):