...
def neuralNetwork(epochs,batch_size,optimizer,learning_rate,beta,beta1,beta2,epsilon,weight_init,num_layers,hidden_size,activation,momentum,loss_function,weight_decay):
      ...
      Y_predicted, _, _ = feedForward(x_testFlat.T, W, b,num_layers,activation)
      predictions = get_predictions(Y_predicted)
      wandb.log({"Confusion_Matrix": wandb.plot.confusion_matrix(preds =predictions,y_true = y_test,class_names =class_name)})


run=wandb.init(project="dl_assignment",entity="cs22m024",reinit='true')
#neuralNetwork(epochs=15,batch_size= 32,optimizer ='adam',learning_rate = 0.01,beta =0.85,beta1 = 0.8,beta2 =0.9,epsilon =1e-7,weight_init='xavier',num_layers=3,hidden_size=256,activation='tanh',momentum=0.9,loss_function='cross_entropy',weight_decay=0)


def confusionMatrix():
  neuralNetwork(epochs=20,batch_size= 64,optimizer ='adam',learning_rate = 0.01,beta =0.85,beta1 = 0.8,beta2 =0.9,epsilon =1e-7,weight_init='random',num_layers=3,hidden_size=256,activation='ReLU',momentum=0.9,loss_function='cross_entropy',weight_decay=0)
